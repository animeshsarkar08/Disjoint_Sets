...
typedef struct HT{
	...
	int a,b;// ax + b ,a = probe_constant != 0
...
int main(int argc,char *argv[]){
	...
	input_table(ht);
    /* insert(&ht,"animesh",1); */
    /* insert(&ht,"sherlock",2); */
    /* insert(&ht,"byomkesh",3); */
    /* insert(&ht,"Len",4); */
    /* insert(&ht,"Sue",5); */
    /* insert(&ht,"Sue",7); */
	/* delete(ht,"animesh"); */
...
HT *create_table(){
    ...
    scanf("%d",&b);
	getchar();
	snprintf(c,7,"%dx+%d",a,b);
	sscanf(c,"%dx+%d",&ht->a,&ht->b);
	if(ht->tablesize<= 0){
...
int probingfun(int a,int b,int x){

	//putting a default case "a = 1" if the user input is "a=0"(invalid)
	if(!a) a = 1;
